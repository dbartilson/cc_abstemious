...
impl Mesh {
    ...
    pub fn read_from_vtk(&mut self, path: &Path) -> std::io::Result<()> {
        if !path.exists() {
            error!(
                "Mesh file not found at specified path: {}",
        ...
        if !(str_buf.contains("# vtk DataFile Version 1.0")
            || str_buf.contains("# vtk DataFile Version 2.0")
            || str_buf.contains("# vtk DataFile Version 3.0"))
        {
                    ...
                    for i in 0..npts {
                        ...
                        for j in 0..3 {
                            reader
                                ...
                                .expect("Unable to process binary VTK: POINTS");
            ...
            // Read CELL data block
                ...
                if format == VTKFormat::ASCII {
                    for i in 0..nelem {
                        ...
                        sline = str_buf.split_whitespace();
                        let npts: usize = sline.next().as_ref().unwrap().parse().unwrap();
                        let mut elem_temp: Element = Element {
                            id: i,
                            body_id: 0,
                    ...
                    for i in 0..nelem {
                        reader
                            .get_reader()
                            .read_exact(&mut bin_buf)
                            .expect("Unable to process binary VTK: CELLS NPTS");
                        ...
                        for _j in 0..npts {
                            reader
                                .get_reader()
                                .read_exact(&mut bin_buf)
                                .expect("Unable to process binary VTK: CELLS NODE_IDS");
                            let uresult = u32::from_be_bytes(bin_buf);
                            elem_temp.node_ids.push(
                                usize::try_from(uresult).expect("Convert usize from u32 failed"),
                            )
                        }
                        elements.push(elem_temp)
                    }
                ...
                if format == VTKFormat::ASCII {
                    ...
                    for elem in elements.iter_mut().take(nelem) {
                        reader
                            .get_reader()
                            .read_exact(&mut bin_buf)
                            .expect("Unable to process binary VTK: CELL_TYPES");
                        let uresult = usize::try_from(u32::from_be_bytes(bin_buf))
                            .expect("Covert usize from u32 failed");
