...
pub fn calculate_surface_power(
    predata: &preprocess::PreData,
    incident: &PrimaryVariables,
    total: &PrimaryVariables,
) -> Power {
...
pub fn write_results_at_frequency(
    predata: &preprocess::PreData,
    ...
    if scatt.len() != points.len() {
        panic!(
            "Error while writing results: pressure array and field point array not equal length!"
        )
    }
...
pub fn write_results_at_point(
    predata: &preprocess::PreData,
    results: &Vec<FPResult>,
    index: usize,
) -> Result<(), Box<dyn Error>> {
...
pub fn write_power(
    ...
    let mut wtr = Writer::from_path(fname)?;
    let _ = wtr.write_record(["freq", "w_scat", "w_inc"]);
    for result in results {
        wtr.write_record(&[
            result.frequency.to_string(),
...
pub fn write_results(
    ...
    if predata.get_output_filename().is_empty() {
        return Ok(());
    }
    ...
    if predata.get_field_points().len() == 1 && !predata.get_output_power_bool() {
        ...
        if predata.get_output_power_bool() {
            let _ = write_power(predata, Some(&ofpower), results);
        }
        Ok(())
    }
}
